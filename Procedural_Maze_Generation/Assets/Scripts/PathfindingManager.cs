...
public class PathfindingManager : MonoBehaviour
{
    ...
    public struct Agent
    {
        ...
        public Queue<Node1> getPath;
        public Agent(Transform agentPos, Queue<Node1> getPath)
        {
    ...
    private Node1 findMinValue(List<Node1> nextNode)
    {

        

        Node1 lowestValue = nextNode[0];
    ...
    private void redneredNodePath(Queue<Node1> getSequence)
    {
    ...
    private void markDiscovered(List<Node1> disPosition) 
    {
    ...
    private IEnumerator agentPatrol(List<Transform> patrolingPoints, Transform start)
    {
      ...
      //  currentNode.Enqueue(start);
        ...
        while (start.transform.position!=Player.transform.position)
        {
            ...
            List<Node1> nextNode = new List<Node1>();
            ...
            if (!hasFoundClosestPoint)
            {
                ...
                {
                    Node1 N = new Node1(patrolingPoints[i].transform.position, true);
            ...
            Node1 closest = findMinValue(nextNode);
            ...
            while (!hasReachedClosest)
            {
                ...
                Node1 nextPoint = agentCurrentPos.getPath.Dequeue();
                ...
                Node1 getCurrentNode = Grid.getNodeposition(agentTransform.transform.position);
                Node1 getClosestNode = Grid.getNodeposition(closest.pos);
    ...
    private IEnumerator followPath( Transform Start, Vector3 Final)
    {
        ...
        while (agent.getPath.Count != 0)
        {
            ...
            Node1 target = agent.getPath.Dequeue();
            while (!hasReached)
            {
